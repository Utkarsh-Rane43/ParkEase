...
def on_park():
    ...
    if entry_gate in entry_points:
        ...
        if spot:
            def after_animation():
                # Redraw the parking space after animation
                draw_parking_space(canvas)
                messagebox.showinfo("Parking Success", f"Car {car_number} parked at {spot}. Distance: {total_distance} units.")
            
            # Start animation and call after_animation upon completion
            animate_car_movement(canvas, path, car_number, after_animation)
        else:
...
def animate_car_movement(canvas, path, car_number, callback=None):
    ...
    # Extract the start and next position from the path
    start = path[0]
    next_position = path[1] if len(path) > 1 else start
    
    # Calculate the movement delta
    x1, y1 = start[1] * 50 + 25, start[0] * 50 + 25  # Center of the start cell
    x2, y2 = next_position[1] * 50 + 25, next_position[0] * 50 + 25  # Center of the next cell
    
    # Create a car (circle) if not already existing
    car = canvas.create_oval(x1 - 10, y1 - 10, x1 + 10, y1 + 10, fill="blue", tags=f"car_{car_number}")
    canvas.tag_raise(car)
    
    # Function to perform the step-wise animation
    def move_step(current_x, current_y, dx, dy, steps_left):
        if steps_left <= 0:
            # Animation for this step is complete
            if len(path) > 1:
                # Remove the previous step and move to the next
                canvas.delete(f"car_{car_number}")
                animate_car_movement(canvas, path[1:], car_number, callback)
            else:
                # Final step - Callback after animation
                if callback:
                    callback()
            return
        
        # Calculate the next position in this step
        canvas.move(f"car_{car_number}", dx, dy)
        canvas.after(10, move_step, current_x + dx, current_y + dy, dx, dy, steps_left - 1)
    
    # Determine the number of steps for smooth animation
    steps = 10
    dx = (x2 - x1) // steps
    dy = (y2 - y1) // steps
    move_step(x1, y1, dx, dy, steps)
